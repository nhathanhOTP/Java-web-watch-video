...
public class AppListener implements HttpSessionListener, ServletContextListener {
	...
	public void contextDestroyed(ServletContextEvent e) {
		...
		try {
			String path = application.getRealPath("/txt/visitors.txt");
	...
	public void contextInitialized(ServletContextEvent e) {
		...
		Integer visitors = 0;
		try {
			String path = application.getRealPath("/txt/visitors.txt"); // đặt tại webroot
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("/views/random.jsp").forward(request, response);
